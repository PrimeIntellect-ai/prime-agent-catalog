...
- We use these sources of truth to generate our model catalog, which keeps track of model pricing and metadata across providers. The generator lives in this repo: `npm run catalog:export` regenerates the catalog from the live upstream endpoints (models.dev, including its `vercel` slug for the Vercel AI Gateway, and OpenRouter)
